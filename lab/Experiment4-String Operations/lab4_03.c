...
float  evalpost(char f[])
{
    ...
    int pos = 0;
     ...
     核心思路就是把中置表达式转化成后缀表达式后，然后把该数组元素从左到右逐一入新栈`obst`，如果遇到了操作符（isop(pos++)），那就执行对应操作，把结果再入栈，然后重复读取后缀字符串数组直到指向为空，
...
int main()
 {
        ...
        int i;
        printf("\n\n请输入中缀表达式:\n");
        gets(e);    //从标准输入设备读字符串函数，其可以无限读取，不会判断上限，以回车结束读取（gcc认为这个函数不安全笑死）
